...
#+PROPERTY: header-args :mkdirp yes :session *guix-system-conf* :noweb yes
...
#+begin_src scheme :tangle .config/guix/channels.scm :results no
(list
 ;; (channel
 ;;       (name 'channel-n)
 ;;       (branch "develop")
...
sudo guix system --cores=$(nproc) -L ~/.config/guix/systems reconfigure ~/.config/guix/systems/umbreon.scm #TODO change this to detect system
#+end_src

** base services
:PROPERTIES:
:CREATED:  [2023-07-01 Sat 13:32]
:ID:       ba3d46ee-d8e0-41de-ba75-dd8c5b9a6eb1
:END:
...
(use-service-modules linux desktop xorg)
...
#+begin_src scheme :noweb yes :tangle .config/guix/systems/base-system.scm :results silent
  ...
  (operating-system
   ...
   (locale-definitions
    (append %default-locale-definitions
            (list (locale-definition (name "en_US.UTF-8") (source "en_US"))
                  (locale-definition (name "ja_JP.UTF-8") (source "ja_JP")))))
   ...
   (users (cons (user-account
                 ...
                 (supplementary-groups '("wheel"     ;; sudo
                                         ...
                                         "kvm"
                                         "libvirt"  ;; managing QEMU/KVM VMs without sudo
                                         "docker"   ;; 'docker' commands without sudo
                                         "tty"
                                         "input"
                                         "adbusers"
                                         "realtime"  ;; Enable realtime scheduling
                                         "lp"        ;; control line printers
...
(operating-system
  (inherit base-operating-system)
  (host-name "umbreon")
  (mapped-devices umbreon-mapped-devices)
  (file-systems umbreon-file-systems)
...
#+begin_src scheme :noweb-ref umbreon-storage :results silent
(define umbreon-mapped-devices
  (list (mapped-device
...
#+begin_src scheme :noweb-ref xorg-tearfree-config :results silent
...
*** flareon
:PROPERTIES:
:CREATED:  [2023-07-22 Sat 14:09]
:ID:       f8720ffa-de88-4f3f-8c94-7e6d9ee79c2c
:END:

~flareon~ is an Asus Aspire Black that was damaged in a car accident, leading to LCD panel and chassis damage that renders the built-in screen useless. It generally serves as a backup desktop/server with an external screen.

The main configuration theme that revolves around this machine is the descrete NVIDA 960M graphics card which requires proprietary drivers to run at full efficiency.

#+begin_src scheme :tangle .config/guix/systems/flareon.scm :results silent
...
(operating-system
  ...
  (bootloader
   (bootloader-configuration
    ...
    (keyboard-layout keyboard-layout)))
  (swap-devices
   (list (uuid "5695793a-706b-49c9-993d-122ca418a882")))
  (file-systems
   (cons* (file-system
            (mount-point "/")
            (device
             (uuid "0567315a-abec-4ecc-8d09-b8de47aa29b0"
                   'ext4))
            (type "ext4"))
          %base-file-systems)))
#+end_src
...
for profile in $profiles; do
  ...
  # 1. Get the base filename (e.g., "video.scm")
  base_filename=$(basename "$profile")
  
  # 2. Strip the .scm extension to get the clean profile name (e.g., "video")
  profileName="${base_filename%.scm}"
