...
#+begin_src elisp
(kill-process (get-process "inotifywait"))
#+end_src
...
(defun keep-output (process output)
  (progn
    (setq kept (cons output kept))
    (sleep-for 0.1)
    (store-sm-element-paths)
    ;; (message %s (store-element-paths))
    ;; (sleep-for 0.3)
    (setq kept nil)))
#+end_src
...
#+RESULTS:
| /home/nanjigen/Documents/SM/neuron/elements/2/16/4/22850.HTM | /home/nanjigen/Documents/SM/neuron/elements/2/16/5/22854.HTM |

#+begin_src elisp
(defun store-sm-element-paths ()
  (interactive)
  (if (> (length kept) 1)
      (let* ((answer-path (nth 0 (split-string (car kept) " ")))
             (answer-file (substring
                           (nth 2 (split-string (car kept) " ")) 0 -1))
             (question-path (nth 0 (split-string (cadr kept) " ")))
             (question-file (substring
                             (nth 2 (split-string (cadr kept) " ")) 0 -1))
             (answer (format "%s%s" answer-path answer-file))
             (question (format "%s%s" question-path question-file)))
        (setq sm-paths (list answer question)))
    (let* ((article-path (nth 0 (split-string (car kept) " ")))
           (article-file (substring
                          (nth 2 (split-string (car kept) " ")) 0 -1))
           (article (format "%s%s" article-path article-file)))
      (setq sm-paths article))))
#+end_src
